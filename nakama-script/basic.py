...
class Error:
    ...
    def as_string(self):
        ...
        result += '\n\n' + strings_with_arrows(self.pos_start.fileTxt, self.pos_start, self.pos_end)
        return result

class illgeal_char_error(Error):
    def __init__(self, pos_start, pos_end, details):
        super().__init__(pos_start, pos_end, 'Illegal', details)
...
class Lexer:
    ...
    def make_token(self):
        ...
        while self.currentChar != None:
            if self.currentChar in ' \t':
                ...
                tokens.append(Token(TOKEN_LPARAN, pos_start=self.pos))
                self.advance()
            elif self.currentChar == ')':
                tokens.append(Token(TOKEN_RPARAN, pos_start=self.pos))
                self.advance()       
            elif self.currentChar == '[':
                ...
                self.advance()
                return [],illgeal_char_error(pos_start, self.pos,"'" + char + "'")
    ...
    def make_string(self):
        ...
        while self.currentChar != None and (self.currentChar != '"'  or escape_character):
            if escape_character:
                string += escape_characters.get(self.currentChar, self.currentChar)
            else:
                if self.currentChar == '\\':
    ...
    def make_identifier(self):
        ...
        while self.currentChar != None and self.currentChar in LETTER_DIGITS + '_':
...
class Parser:
    ...
    def statement(self):
        ...
        if res.error:
            return res.failure(invalid_syntax_error(
                self.current_tok.pos_start, self.current_tok.pos_end,
                "Expected 'return', 'continue', 'break', 'nakama', 'if', 'for', 'while', 'gears', int, float, identifier, '+', '-', '(', '[' or 'NOT'"
            ))
    ...
    def list_expr(self):
        ...
        if self.current_tok.type == TOKEN_RSQUARE:
            ...
            if res.error:
                return res.failure(invalid_syntax_error(
                    self.current_tok.pos_start, self.current_tok.pos_end,
                    "Expected ']', 'nakama', 'if', 'for', 'while', 'gears', int, float, identifier, '+', '-', '(', '[' or 'NOT'"
                ))
    ...
    def if_expr_b_or_c(self):
        ...
        if self.current_tok.matches(TOKEN_KEYWORD, 'elseif'):
    ...
    def for_expr(self):
        ...
        if self.current_tok.matches (TOKEN_KEYWORD, 'raise'):
            res.register_advancement()
            self.advance()
    ...
    def atom(self):
        ...
        elif tok.type == TOKEN_LPARAN:
            res.register_advancement()
            self.advance()
            expr = res.register(self.expr())
            if res.error: return res
            if self.current_tok.type == TOKEN_RPARAN:
                res.register_advancement()
                self.advance()
                return res.success(expr)
            else:
        ...
        elif tok.matches(TOKEN_KEYWORD, 'gears'):
        ...
        return res.failure(invalid_syntax_error(
			...
			"Expected int, float, identifier, '+', '-', '(', '[', if', 'for', 'while', 'gears'"
		))
    ...
    def power(self):
        return self.binaryOp(self.call, (TOKEN_POWER, ), self.factor)
    ...
    def call(self):
        ...
        if self.current_tok.type == TOKEN_LPARAN:
            res.register_advancement()
            self.advance()
            arg_nodes = []
    ...
    def term(self):
        return self.binaryOp(self.factor, (TOKEN_MULTIPLY, TOKEN_DIVIDE)) 
    
    def arith_expr(self):
        return self.binaryOp(self.term, (TOKEN_PLUS, TOKEN_MINUS))
    ...
    def comp_expr(self):
        ...
        node = res.register(self.binaryOp(self.arith_expr, (TOKEN_EE, TOKEN_NE, TOKEN_LT, TOKEN_GT, TOKEN_LTE, TOKEN_GTE)))
    ...
    def expr(self):
        ...
        if self.current_tok.matches(TOKEN_KEYWORD, 'nakama'):
            res.register_advancement()
            self.advance()
        ...
        node = res.register(self.binaryOp(self.comp_expr, ((TOKEN_KEYWORD, 'AND'), (TOKEN_KEYWORD, 'OR'))))
    ...
    def func_def(self):
        ...
        if not self.current_tok.matches(TOKEN_KEYWORD, 'gears'):
            return res.failure(invalid_syntax_error(
                self.current_tok.pos_start, self.current_tok.pos_end,
                f"Expected 'gears'"
            )) 
...
global_symbol_table.set("is_gears", BuiltInFunction.is_function)
